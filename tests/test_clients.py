...
CLIENTS = {
    "v1": lambda: NacosClientV1("localhost", 8848, "public"),
...
CALL_KWARGS = {
    ...
    "get_config_history": dict(
        nid=5, data_id="app.yaml", group_name="DEFAULT_GROUP", namespace_id=NS
    ),
    "get_config_previous": dict(
        config_id=7, data_id="app.yaml", group_name="DEFAULT_GROUP", namespace_id=NS
    ),
...
async def test_client_routing(version, method, http_mock):
    ...
    assert captured["method"] == exp_method, (
        f"[{version}.{method}] 方法应为 {exp_method}，实际 {captured['method']}"
    )
    assert captured["path"] == exp_path, (
        f"[{version}.{method}] 路径应为 {exp_path}，实际 {captured['path']}"
    )

    # 参数键断言（仅当预期非空时，避免对无参接口误报）
    missing = [k for k in exp_keys if k not in captured["params"]]
    assert not missing, (
        f"[{version}.{method}] 缺少参数键 {missing}；实际 {sorted(captured['params'])}"
    )

    # 返回值类型断言
    assert isinstance(result, RETURN_TYPE[method]), (
        f"[{version}.{method}] 返回类型应为 "
        f"{RETURN_TYPE[method].__name__}，实际 {type(result).__name__}"
    )
